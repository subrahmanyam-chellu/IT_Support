...
const Testing = () => {
    ...
    const handleTest = async () => {
        try {
            const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/`, { issue: { issue } });
            if (response.status = 200) {
                setData(response.data);
            }
        } catch (err) {
            setError([true, err.body]);
        }
    }
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: { xs: '360px', md: '800px' }, mx: { xs: '20px', md: '352px' } }}>
                ...
                <Box sx={{
                    ...
                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, maxHeight: { xs: 45, md: 80 } }}>
                        <TextField sx={{ width: { xs: '337px', md: '650px' }, border: '2px solid green', borderRadius: '10px', m: '10px', '& .MuiOutlinedInput-root':{color:'green'} }} onChange={(e)=>{setIssue(e.target.value)}}>Enter the issue description...</TextField>
