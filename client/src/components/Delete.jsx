...
const Delete = ({token}) => {
    ...
    return (
        <Box>
            <Typography color='error' sx={{ fontSize: { xs: '', md: 35 }, fontWeight: 600, mb: '15px' }}>Delete Tickets using below</Typography>
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: { md: 'space-between' }, alignItems: { xs: 'flex-start', md: 'center' }, mb: '20px' }}>
                ...
                <Button variant='contained' sx={{
                    width: { md: '180px' }, height: { md: '55px' }, fontSize: { xs: '', md: 30 }, fontWeight: 600,
                    textTransform: 'none', backgroundColor: '#af462b', borderRadius: '25px'
                }} onClick={() => { deleteHandle(ticketId); }}>Delete</Button>
            </Box>
            <Button variant='contained' sx={{
                width: { md: '260px' }, height: { md: '55px' }, fontSize: { xs: '', md: 30 }, fontWeight: 600,
